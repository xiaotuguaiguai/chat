...
//											Toast.makeText(getApplicationContext(), st12, Toast.LENGTH_SHORT).show();
	...
	private void clearGroupHistory() {
		...
		Toast.makeText(this, "聊天记录已被清空", Toast.LENGTH_SHORT).show();
	}
				...
				new EMCallBack() {
					...
					public void onError(int code, final String error) {
						runOnUiThread(new Runnable() {
							...
							public void run() {
								progressDialog.dismiss();
//								Toast.makeText(GroupDetailsActivity.this, "", Toast.LENGTH_SHORT).show();
							}
						});
					}
            ...
            public void run() {
                try {
					...
					runOnUiThread(new Runnable() {
						...
						public void run() {
							progressDialog.dismiss();
							Toast.makeText(GroupDetailsActivity.this, "失败", Toast.LENGTH_SHORT).show();
						}
					});
                }
            }
