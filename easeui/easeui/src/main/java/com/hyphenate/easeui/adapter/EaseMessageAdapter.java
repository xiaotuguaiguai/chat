...
import com.hyphenate.easeui.widget.presenter.EaseChatMoneyPresenter;
import com.hyphenate.easeui.widget.presenter.EaseChatMoneyTipPresenter;
...
public class EaseMessageAdapter extends BaseAdapter{
	...
	private static final int MESSAGE_TYPE_RECV_MONEY= 14;
	private static final int MESSAGE_TYPE_SENT_MONEY = 15;

	private  static final int MESSAGE_TYPE_RECV_MINGPIAN_CALL=17;
	private  static final int MESSAGE_TYPE_SENT_MINGPIAN_CALL=18;


	private  static final int MESSAGE_TYPE_RECV_MONEYTIP_CALL=19;
	private  static final int MESSAGE_TYPE_SENT_MONEYTIP_CALL=20;
	...
	public int getItemViewType(int position) {
		...
		if (message.getType() == EMMessage.Type.TXT) {
		    ...
		    }else if(message.getBooleanAttribute(EaseConstant.MESSAGE_ISMONEY,false)){//自加红包
				return message.direct()==EMMessage.Direct.RECEIVE?MESSAGE_TYPE_RECV_MONEY : MESSAGE_TYPE_SENT_MONEY;

			}else if(message.getBooleanAttribute(EaseConstant.IS_CARD_TYPE,false)){//自加mingpian
				return message.direct()==EMMessage.Direct.RECEIVE?MESSAGE_TYPE_RECV_MINGPIAN_CALL : MESSAGE_TYPE_SENT_MINGPIAN_CALL;
			}else if(message.getBooleanAttribute(EaseConstant.IS_MONEY_TIP,false)){//自加收取红包提示
				return message.direct()==EMMessage.Direct.RECEIVE?MESSAGE_TYPE_RECV_MONEYTIP_CALL : MESSAGE_TYPE_SENT_MONEYTIP_CALL;
			}
	...
	protected EaseChatRowPresenter createChatRowPresenter(EMMessage message, int position) {
        ...
        case TXT:
            ...
            }else if(message.getBooleanAttribute(EaseConstant.MESSAGE_ISMONEY,false)){
				presenter = new EaseChatMoneyPresenter();
			}else if(message.getBooleanAttribute(EaseConstant.IS_CARD_TYPE,false)){
				presenter = new EaseChatCardPresenter();
			}else if(message.getBooleanAttribute(EaseConstant.IS_MONEY_TIP,false)){
				presenter = new EaseChatMoneyTipPresenter();
			}
			else{
