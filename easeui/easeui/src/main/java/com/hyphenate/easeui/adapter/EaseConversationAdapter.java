...
//        holder.list_itease_layout.setBackgroundResource(R.drawable.ease_mm_listitem);
        ...
        if(conversation.getExtField().equals("toTop")){
            holder.list_itease_layout.setBackgroundColor(context.getResources().getColor(R.color.top_color));

        }else{
            holder.list_itease_layout.setBackgroundColor(context.getResources().getColor(R.color.white_color));
        ...
        String fromNickName="",myNickName="",fromPhoto="",myPhoto="";
        try {
            fromNickName=    lastMessage.getStringAttribute(EaseConstant.ATTR_NICKNAME);
            myNickName=    lastMessage.getStringAttribute(EaseConstant.ATTR_TONICKNAME);
            fromPhoto=    lastMessage.getStringAttribute(EaseConstant.ATTR_PHOTO);
            myPhoto=    lastMessage.getStringAttribute(EaseConstant.ATTR_TOPHOTO);
            Log.e("ss",fromNickName);
...
//            Glide.with(getContext()).load(fromPhoto).apply(options).into(holder.avatar);
        ...
        if(!EaseSharedUtils.isEnableMsgRing(context,EaseConstant.USER_ID,username)){
//            conversation.markMessageAsRead(username);
            EMClient.getInstance().chatManager().getUnreadMessageCount();
        }

        if (conversation.getUnreadMsgCount() > 0 && EaseSharedUtils.isEnableMsgRing(context,EaseConstant.USER_ID,username)) {
    ...
    private int  unread(){
        ...
        while (var3.hasNext()){
            ...
            if(conversation._getType() != EMAConversation.EMAConversationType.CHATROOM &&
                    EaseSharedUtils.isEnableMsgRing(context,  EMClient.getInstance().getCurrentUser(),conversation.conversationId())){
